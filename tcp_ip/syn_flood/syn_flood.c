...
int main(int argc,char** argv){
        ...
        if(inet_aton("219.216.110.220",&target.sin_addr)==0)
        {
...
//                inet_aton("219.216.87.22",&ip->ip_src);
                tcp->check=check_sum((unsigned short*)tcp,sizeof(struct tcphdr));
                if(-1 == sendto(skfd,buf,ip_len,0,(struct sockaddr*)target,sizeof(struct sockaddr_in)))
                {
                    perror("sendto");
                }
               // usleep(100000);
